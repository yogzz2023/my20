    ...
    if i == 0:
        ...
        for cluster in clusters:
            ...
            for i in range(2 ** num_measurements):
                binary_rep = bin(i)[2:].zfill(num_measurements)
                hypothesis = []
                for j, bit in enumerate(binary_rep):
                    if bit == '1':
                        hypothesis.append(cluster[j])
                if len(hypothesis) == num_targets:
                    hypotheses.append(hypothesis)
...
plt.scatter(filtered_values_csv[:, 0], result, label='filtered range (track id 57)', color='red', marker='*')
